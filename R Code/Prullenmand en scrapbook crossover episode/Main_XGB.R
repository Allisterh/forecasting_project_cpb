    ...
    for (f in 1:25){  
      ...
      if(f==1 || f%%24==0){
        ...
        tune_control <- caret::trainControl(
          ...
          returnResamp = "all" #Use same sample
        )
        
        # Train and tune the XGBoost model using grid search
        trainxy <- xgb.DMatrix(data = as.matrix(train_x), label = as.matrix(train_y))
        #print(trainxy)
        #train_y <- factor(train_y, labels = c("yes", "no"))
        ...
        #   verboseIter = FALSE, #Dont print results
        #   returnResamp = "all" #Use same sample
        # )
        # 
        # 
      ...
      print(y_real[f])
      predicted_DeltaY <- predict(XGB_tuned, xgb_test)
