...
# - R-Squared Function -
n<-120 #sample size
K_small <- c(117, 65, 18, 108,182,135,225,83,173,126,200,290,243,191,308)

Rsquared_function <- function(y_real, y_pred, k){
  e_p <- sum((y_real - y_pred)^2)
  e_m <- sum((y_real - mean(y_real))^2)
  Rsquared <- 1-(e_p/e_m)
  Rsquared <- 1 - ((n-1)/(n-k-1))*(1-Rsquared)
...
marginal_function <- function(horizons, y_real, Mf_matrices, F_matrices, k1, k2){
  ...
  for (f in 1:n_marg_combinations){
    ...
    for (h in 1:length(horizons)){
      Rsquared_Mf[f,h] <- Rsquared_function(y_real, y_pred_Mf[,h], k1[f]) #calculate Rsquared including feature
      Rsquared_F[f,h] <- Rsquared_function(y_real, y_pred_F[,h], k2[f])  #calculate Rsquared excluding feature
    }
  }
  #print(Rsquared_Mf)
  #print(Rsquared_F)
...
k1 <- K_small[c(5,8,9,11,12,14,15)]
k2 <- K_small[c(1,3,4,6,7,10,13)]

RF_Small_F_marginal_Rsquared <- marginal_function(horizons, y_real, Mf_matrices, F_matrices, k1, k2)
...
k1 <- K_small[c(6,8,10,11,13,14,15)]
k2 <- K_small[c(1,2,4,5,7,9,12)]

RF_Small_MAF_marginal_Rsquared <- marginal_function(horizons, y_real, Mf_matrices, F_matrices, k1, k2)
...
k1 <- K_small[c(7,10,9,13,14,15,12)]
k2 <- K_small[c(1,3,2,6,8,11,5)]

RF_Small_MARX_marginal_Rsquared <- marginal_function(horizons, y_real, Mf_matrices, F_matrices, k1, k2)
